  ...
  while (1)
  {
    ...
    switch (fid)
    {
      case 1:
        if (ftype == T_I32)
        {
          if ((ret = thrift_protocol_read_i32 (protocol, &this_object->what, error)) < 0)
            return -1;
          xfer += ret;
...
invalid_operation_write (ThriftStruct *object, ThriftProtocol *protocol, GError **error)
{
  ...
  if ((ret = thrift_protocol_write_struct_begin (protocol, "InvalidOperation", error)) < 0)
    return -1;
  xfer += ret;
  if ((ret = thrift_protocol_write_field_begin (protocol, "what", T_I32, 1, error)) < 0)
    return -1;
  xfer += ret;
  if ((ret = thrift_protocol_write_i32 (protocol, this_object->what, error)) < 0)
    return -1;
  if ((ret = thrift_protocol_write_field_end (protocol, error)) < 0)
    return -1;
  xfer += ret;
                                ...
                                GParamSpec *pspec)
{
  ...
  switch (property_id)
  {
    case PROP_INVALID_OPERATION_WHAT:
      self->what = g_value_get_int (value);
                                ...
                                GParamSpec *pspec)
{
  ...
  {
    case PROP_INVALID_OPERATION_WHAT:
      g_value_set_int (value, self->what);
      break;
